package com.hwaggLee.jsoup.utils;
...
*/
	...
	public static Document getDocument(String path,int outtime)throws Exception{
		...
		while(i<5){//出现异常休眠1秒,重新刷新,共刷新5次
			try {
				...
				e.printStackTrace();
				logger.error("url req jsoup error:"+e.getMessage()+";path="+path);
				if( i >= 5 ){
	...
	public static Document getDocument(String path)throws Exception{
		...
		Document doc = null;
		try {
			doc = Jsoup.connect(path).header("User-Agent", "Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10.4; en-US; rv:1.9.2.2) Gecko/20100316 Firefox/3.6.2").get();  
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("url req jsoup error:"+e.getMessage()+";path="+path);
		}
