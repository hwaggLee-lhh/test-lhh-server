...
@Repository("managercwsjcashflowstatement")
public class ManagerCWSJCashFlowStatement extends BaseManager<ModelCWSJCashFlowStatement>{

	@Override
	public Class<ModelCWSJCashFlowStatement> getModelClass() {
		return ModelCWSJCashFlowStatement.class;
	}
	

	public Page<ModelCWSJCashFlowStatement> findPage(ModelCWSJCashFlowStatement example, PageParam pageParam) {
...
//        dc.addOrder(Order.desc("dataCompiledDate"));
	...
	public ModelCWSJCashFlowStatement findByStockCodeAndEndDate(String stockCode,Date endDate) {
		DetachedCriteria dc = super.getDetachedCriteria();
		dc.add(Restrictions.eq("stockCode", stockCode));
		dc.add(Restrictions.eq("endDate", endDate));
		List<ModelCWSJCashFlowStatement> list = super.findList(dc);
