...
@Repository("managercwsjincomestatements")
public class ManagerCWSJIncomeStatements extends BaseManager<ModelCWSJIncomeStatements>{

	@Override
	public Class<ModelCWSJIncomeStatements> getModelClass() {
		return ModelCWSJIncomeStatements.class;
	}
	


	public Page<ModelCWSJIncomeStatements> findPage(ModelCWSJIncomeStatements example, PageParam pageParam) {
...
//        dc.addOrder(Order.desc("dataCompiledDate"));
	...
	public ModelCWSJIncomeStatements findByStockCodeAndEndDate(String stockCode,Date endDate) {
		DetachedCriteria dc = super.getDetachedCriteria();
		dc.add(Restrictions.eq("stockCode", stockCode));
		dc.add(Restrictions.eq("endDate", endDate));
		List<ModelCWSJIncomeStatements> list = super.findList(dc);
