...
public class StartPoiExcel {
	...
	public static void x4()throws Exception{
		...
		for (int x = 0; x < s.length; x++) {
			...
			List<ArrayList<Object>> list = new UtilsExcelReader().processExcelLists(path, sheelname);
	...
	public static void x2() throws Exception {
		...
		if (!file.exists())
			return;
		List<String> lists =  UtilsFile.readChildFiles(file);
		System.out.println("------------------>数据开始");
		for (int i = 0; i < lists.size(); i++) {
			List<ArrayList<Object>> objList = new UtilsExcelReader().processExcelLists(lists.get(i), "sheet1");
	...
	public static void x1() {
		...
		List<ArrayList<Object>> objList = new UtilsExcelReader().processExcelLists(path,
				sheetName);
		if (objList == null || objList.size() == 0) {
			System.out.println("EXCEL null");
			return;
